...
from backend.app.schemas import Carousel, Slide, SlideType
...
def get_tailwind_cli_path() -> Optional[Path]:
    """Locate the standalone Tailwind CLI binary."""
    candidates = [
...
def render_slide_html(
    ...
    template_name = f"{slide.slide_type.value}.html"
    template = jinja_env.get_template(template_name)

    highlighted_code = ""
    if slide.code:
        lang = slide.language.value if slide.language else carousel.topic.language.value
        highlighted_code = highlight_code(slide.code, language=lang, show_line_numbers=True)
    ...
    return template.render(
        ...
        carousel_author=carousel.author,
    )
...
def render_carousel(
    ...
    Args:
        carousel: Carousel Pydantic model instance.
